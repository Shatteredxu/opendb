...
type (
	...
	hashIndex struct {
		mu      *sync.RWMutex
		trees   map[string]*art.AdaptiveRadixTree//自适应基数树(ARTree)
		idxTree *art.AdaptiveRadixTree
	}
...
func (db *RoseDB) writeLogEntry(ent *logfile.LogEntry, dataType DataType) (*valuePos, error) {
	...
	activeLogFile := db.getActiveLogFile(dataType)//获取当前活跃可写入的文件
	if activeLogFile == nil {
		return nil, ErrLogFileNotFound
	}

	opts := db.opts
	entBuf, esize := logfile.EncodeEntry(ent)//将entry编码为header+key+value的形式
	if activeLogFile.WriteAt+int64(esize) > opts.LogFileSizeThreshold {//大于每个文件规定的最大写入长度LogFileSizeThreshold
...
/**
	loadLogFiles 加载本地存储文件

 */
func (db *RoseDB) loadLogFiles() error {
	db.mu.Lock()
	defer db.mu.Unlock()
	fileInfos, err := ioutil.ReadDir(db.opts.DBPath)//读取路径下的所有文件
	if err != nil {
		return err
	}
	...
	for _, file := range fileInfos {
		if strings.HasPrefix(file.Name(), logfile.FilePrefix) {//选择带有log.的文件进行查找
			splitNames := strings.Split(file.Name(), ".")
			fid, err := strconv.Atoi(splitNames[2])
			if err != nil {
				return err
			}
			typ := DataType(logfile.FileTypesMap[splitNames[1]])//hash对应的是下标为2
...
func (db *RoseDB) encodeKey(key, subKey []byte) []byte {
	header := make([]byte, encodeHeaderSize)//encodeHeaderSize = 10
	var index int
...
func (db *RoseDB) handleLogFileGC() {
	...
	ticker := time.NewTicker(db.opts.LogFileGCInterval)//NewTicker可以达到固定时间后向channel发送一个通知
	defer ticker.Stop()
	for {
		select {
		case <-ticker.C://定时执行GC
			if atomic.LoadInt32(&db.gcState) > 0 {//获取数据库此时的状态
				logger.Warn("log file gc is running, skip it")
				break
			}
			for dType := String; dType < logFileTypeNum; dType++ {
				go func(dataType DataType) {//DataType代表数据类型 0为string 1为list等等
...
func (db *RoseDB) doRunGC(dataType DataType, specifiedFid int, gcRatio float64) error {
	...
	maybeRewriteStrs := func(fid uint32, offset int64, ent *logfile.LogEntry) error {
		...
		if node != nil && node.fid == fid && node.offset == offset {
			// rewrite entry
			valuePos, err := db.writeLogEntry(ent, String)//写入文件
			if err != nil {
				return err
			}
			// 写入内存索引
