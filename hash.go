...
func (db *RoseDB) HSet(key, field, value []byte) error {
	...
	hashKey := db.encodeKey(key, field)//根据key, field进行编码
	ent := &logfile.LogEntry{Key: hashKey, Value: value}//先写key，value再写内存，这里构建entry
	valuePos, err := db.writeLogEntry(ent, Hash)//再将entry写入file中
	if err != nil {
		return err
	}
	...
	return db.updateIndexTree(entry, valuePos, true, Hash)//更新树索引
}
